...
class Render extends \Magento\Framework\Pricing\Render
{
    ...
    public function render($priceCode, SaleableInterface $saleableItem, array $arguments = [])
    {
        ...
        if ($this->dataHelper->showPrice($saleableItem, $customerGroupId) || $saleableItem->getTypeId() == 'configurable') {
        ...
        if ($priceReplace) {
            if ($arguments['zone'] == 'item_view' || $arguments['zone'] == 'item_list') {
                if (isset($arguments['price_type_code']) && $arguments['price_type_code'] == 'final_price'
                    ...
                    {
                        $html = sprintf(
                            '<div class="price-box price-final_price" data-role="priceBox" data-product-id="%s" data-price-box="product-id-%s"><a class="not2order-price" href="%s">%s</a></div>',
                            $saleableItem->getId(),
                            $saleableItem->getId(),
                            $this->getUrl('contact'),
