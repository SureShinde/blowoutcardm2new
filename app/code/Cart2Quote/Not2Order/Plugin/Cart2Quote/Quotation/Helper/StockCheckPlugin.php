...
class StockCheckPlugin extends \Magento\Framework\App\Helper\AbstractHelper
{
     ...
     * @throws \Exception
     */
    public function beforeIsMoveToCartAllowed(\Cart2Quote\Quotation\Helper\StockCheck $subject, $result)
    {
        if ($this->moduleManager->isOutputEnabled('Cart2Quote_Not2Order')) {
            ...
            foreach ($visibleItems as $visibleItem) {
                ...
                if ($hideOrderButton) {
                    throw new \Exception(
                        __(
                            "The product '%1' is not orderable and has not been added to your shopping cart.",
                            $product->getName()
                        )
                    );
                }
