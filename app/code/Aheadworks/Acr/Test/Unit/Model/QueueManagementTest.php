...
class QueueManagementTest extends TestCase
{
    ...
    public function testCancelByIdOnException()
    {
        ...
        $this->queueRepositoryMock->expects($this->once())
            ->method('get')
            ->with($queueId)
            ->willThrowException(new LocalizedException(__('Unable to cancel.')));
    ...
    public function testSendByIdOnException()
    {
        ...
        $this->queueRepositoryMock->expects($this->once())
            ->method('get')
            ->with($queueId)
            ->willThrowException(new LocalizedException(__('Unable to send.')));
        $this->expectException(LocalizedException::class);
     ...
     * @param string|null $savedSubject
     * @param string|null $savedContent
     * @param int|null $ruleId
     * @dataProvider getPreviewDataProvider
     */
    public function testGetPreview($storeId, $recipientName, $recipientEmail, $savedSubject = null, $savedContent = null, $ruleId = null)
    {
        $queueData = [
            'store_id' => $storeId,
            'recipient_name' => $recipientName,
            'recipient_email' => $recipientEmail,
            'saved_subject' => $savedSubject,
        ...
        if ($queueData['saved_content']) {
            ...
            $previewMock->expects($this->once())
                ...
                ->willReturnSelf();
            $this->previewFilterMock->expects($this->any())
                ->method('filter')
