...
class InstantProvider
{
    ...
    {
        $buckets = [];
        if (filter_input(INPUT_GET, 'buckets', FILTER_DEFAULT , FILTER_REQUIRE_ARRAY)) {
            $buckets = array_merge($buckets, filter_input(INPUT_GET, 'buckets', FILTER_DEFAULT , FILTER_REQUIRE_ARRAY));
        }
